...
const firebaseConfig = {
  ...
  databaseURL:
    "https://cs353-team-18-default-rtdb.europe-west1.firebasedatabase.app",
...
function submitRecipe(req, res) {
  ...
  newRecipeRef
    .then(() => {
      ...
      res
        .status(200)
        .send(`Recipe submitted successfully with key: ${recipeKey}`);
    })
    .catch((error) => {
      console.error(error);
...
async function submitImage(req, res) {
  ...
  req.on("end", () => {
    ...
    if (header === "89504e47") {
      ...
      res.status(400).send("Images must be JPEG or PNG.");
      return;
    }
    ...
    // format like a uuid
    const uuid = `${md5.slice(0, 8)}-${md5.slice(8, 12)}-${md5.slice(
      12,
      16
    )}-${md5.slice(16, 20)}-${md5.slice(20, 32)}`;
